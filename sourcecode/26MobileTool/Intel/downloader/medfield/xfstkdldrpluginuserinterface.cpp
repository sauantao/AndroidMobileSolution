...
void attachFunction(void* clientData)
{
    ...
    plugin->ui->DeviceStatus->setText(QString("MEDFIELD TARGETS DETECTED: %1").arg(numtargets));
    ...
    ui(new Ui::XfstkDldrPluginUserInterface)
{
    ...
    this->dldrapi->registerdevicecallback(&this->CallbackStruct,MEDFIELD);
#else
    ...
    this->ui->DeviceStatus->setText(QString("MEDFIELD TARGETS DETECTED: 0"));
...
void XfstkDldrPluginUserInterface::OptionsInterfaceChanged(OptionsInterfaceState *State)
{
    if(!this->CurrentState.IsDownloadThreadDone) {
        ...
        if(State->ProvisionCount == QString("0")) {
            ...
            this->ui->ProvisionCount->setText(QString("Medfield Provisions Attempted: %1").arg(this->CurrentState.ProvisionCount));
        }
    ...
    this->SaveUserInterfaceStateToDisk(QString("Intel"),QString("Medfield"));
    return;
}
...
#ifndef XFSTK_OS_WIN
    xfstkdldrapi status;
    int numtargets = status.getavailabletargets(MEDFIELD);
#else
    int numtargets = this->socDevs.number_devices;

#endif
    this->ui->DeviceStatus->setText(QString("MEDFIELD TARGETS DETECTED: %1").arg(numtargets));
...
void XfstkDldrPluginUserInterface::BeginDownload()
{
    ...
    this->MiscDnXPath = this->CurrentState.MiscDnXPath;//this->ui->MiscDnXPath->text();
    ...
    if(this->CurrentState.MiscDnxEnable && MiscDnXPath != "N/A") {
        if(!this->VerifyBinaryFilePath(MiscDnXPath)) {
            QMessageBox Message;
            Message.setText("Please select valid MiscDnx image from option settings.");
            Message.exec();
            return;
        }
    } else if(this->CurrentState.MiscDnxEnable && MiscDnXPath == "N/A") {
        QMessageBox Message;
        Message.setText("Please select valid MiscDnx image from option settings.");
        Message.exec();
        return;
