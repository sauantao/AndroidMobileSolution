...
extern QString GlobalCloverviewPlusMessageString;
extern QString GlobalCloverviewPlusStatusValue;
extern int GlobalCloverviewPlusProgressValue;
extern QStringList GlobalCloverviewPlusStatusLog;
...
void attachFunction(void* clientData)
{
    ...
    plugin->ui->DeviceStatus->setText(QString("CLOVERVIEWPLUS TARGETS DETECTED: %1").arg(numtargets));
...
{
    this->ui->BeginDownload->setText(QString::fromUtf8("Bắt đầu chạy"));
    this->ui->DownloadDetails->setText(QString::fromUtf8("Không có hoạt động No provisioning activities in progress..."));
...
XfstkDldrPluginUserInterface::XfstkDldrPluginUserInterface(QTabWidget *parent, MainWindow *window) :
	TabWidgetBase(0, tr("&Intel Widget"), parent),
	...
	main_window_(window)
{
    ...
    temp.append(QString(" - "));
    temp.append("Intel-LOG--No provisioning activities in progress...");
    ...
    this->dldrapi->registerdevicecallback(&this->CallbackStruct,CLOVERVIEWPLUS);
#else
    ...
    this->ui->DeviceStatus->setText(QString("CLOVERVIEWPLUS TARGETS DETECTED: 0"));
...
void XfstkDldrPluginUserInterface::OptionsInterfaceChanged(OptionsInterfaceState *State)
{
    if(!this->CurrentState.IsDownloadThreadDone) {
        ...
        if(State->ProvisionCount == QString("0")) {
            ...
            this->ui->ProvisionCount->setText(QString("CloverviewPlus Provisions Attempted: %1").arg(this->CurrentState.ProvisionCount));
        }
    ...
    this->SaveUserInterfaceStateToDisk(QString("Intel"),QString("CloverviewPlus"));
...
void XfstkDldrPluginUserInterface::SaveUserInterfaceStateToDisk(QString Vendor, QString ProductPlugin)
{
    ...
    settings.beginGroup(ProductPlugin);
        ...
        settings.beginGroup("Download Options");
            ...
            settings.setValue("SoftfuseInclude", this->CurrentState.SoftfuseInclude);
            if(this->CurrentState.SoftfuseInclude) {
                settings.setValue("SoftfusesPath", this->CurrentState.SoftfusesPath);
            } else {
                settings.setValue("SoftfusesPath", QString("N/A"));
            }
        settings.endGroup();
    ...
    if(settings.status() == QSettings::NoError) {
        settings.beginGroup(ProductPlugin);
            ...
            settings.beginGroup("Download Options");
                ...
                this->CurrentState.SoftfuseInclude = settings.value("SoftfuseInclude", false).toBool();
                this->CurrentState.SoftfusesPath = settings.value("SoftfusesPath", "N/A").toString();
            settings.endGroup();
            settings.beginGroup("Provision Count");
...
void XfstkDldrPluginUserInterface::RestoreUserInterfaceFromCurrentState()
{
    ...
    this->ui->OSDnXBin->setText(this->CurrentState.OSDnXBin);
    this->ui->OSImageBin->setText(this->CurrentState.OSImageBin);
//    this->ui->SoftfuseBin->setText(this->CurrentState.SoftfuseBin);
...
void XfstkDldrPluginUserInterface::ThreadComplete(bool result)
{
    ...
    this->CurrentState.IsDownloadThreadDone = true;
    if(this->CurrentState.StatusListPrevLength < GlobalCloverviewPlusStatusLog.length()) {
        this->CurrentState.StatusListLocation = this->CurrentState.StatusListPrevLength;
        this->CurrentState.StatusListPrevLength = GlobalCloverviewPlusStatusLog.length();
        if(this->CurrentState.ShowDownloadDetails) {
            int i = 0;
            for(int i = this->CurrentState.StatusListLocation; i < this->CurrentState.StatusListPrevLength;i++) {
                this->ui->StatusLog->addItem(QString("%1").arg(GlobalCloverviewPlusStatusLog.at(i)));
            }
...
#ifndef XFSTK_OS_WIN
    xfstkdldrapi status;
    int numtargets = status.getavailabletargets(CLOVERVIEWPLUS);
#else
    int numtargets = this->socDevs.number_devices;

#endif
    this->ui->DeviceStatus->setText(QString("CLOVERVIEWPLUS TARGETS DETECTED: %1").arg(numtargets));
...
void XfstkDldrPluginUserInterface::UpdateUiStatus()
{
    if(this->CurrentState.IsDownloadThreadDone) {
        ...
        this->ui->DownloadDetails->setText(QString("%1").arg(GlobalCloverviewPlusStatusValue));
        this->ui->DownloadProgress->setValue(GlobalCloverviewPlusProgressValue);
        if(this->CurrentState.StatusListPrevLength < GlobalCloverviewPlusStatusLog.length()) {
            this->CurrentState.StatusListLocation = this->CurrentState.StatusListPrevLength;
            this->CurrentState.StatusListPrevLength = GlobalCloverviewPlusStatusLog.length();
            if(this->CurrentState.ShowDownloadDetails) {
...
void XfstkDldrPluginUserInterface::BeginDownload()
{
    ...
    this->SoftfusesPath = this->CurrentState.SoftfusesPath;
    ...
    this->baSoftfusePath = this->CurrentState.SoftfusesPath.toLatin1();
    ...
    this->c_str_SoftfusePath = baSoftfusePath.data();
    this->CurrentState.SuppressErrors = true;
    this->ui->StatusLog->clear();
    GlobalCloverviewPlusStatusLog.clear();
    this->CurrentState.StatusListLocation = 0;
    ...
    if(this->CurrentState.SoftfuseInclude && SoftfusesPath != "N/A") {
        if(!this->VerifyBinaryFilePath(SoftfusesPath)) {
            QMessageBox Message;
            Message.setText("Please select valid Softfuse file from option settings.");
            Message.exec();
            return;
        }
    } else if(this->CurrentState.SoftfuseInclude && SoftfusesPath == "N/A") {
        QMessageBox Message;
        Message.setText("Please select valid Softfuse file from option settings.");
        Message.exec();
        return;
    ...
    this->CurrentState.SuppressErrors = false;
    GlobalCloverviewPlusStatusValue = this->ui->DownloadDetails->text();
    ...
    if(this->CurrentState.IsDownloadThreadDone) {
        ...
        if(this->CurrentState.SoftfuseInclude == true) {
            xfstknewthreadobj.bSoftfuseInclude = true;
            xfstknewthreadobj.SoftfusesPath = this->CurrentState.SoftfusesPath;
        } else {
            xfstknewthreadobj.bSoftfuseInclude = false;
            xfstknewthreadobj.SoftfusesPath = "N/A";
        }
...
void XfstkDldrPluginUserInterface::LockOnUI()
{
	//enableControl(false);
}

void XfstkDldrPluginUserInterface::DoFinished()
{
	//enableControl(true);
}

void XfstkDldrPluginUserInterface::UpdateUI()
{
	ui->retranslateUi(this);
}

void XfstkDldrPluginUserInterface::SetTabLabel(QTabWidget *tab_widget, int index)
{
...
void XfstkDldrPluginUserInterface::SetShortCut(int, const QString &)
{

}


void XfstkDldrPluginUserInterface::on_pushButton_Setting_clicked()
{

	//this->SettingWidget.setModal(true);
