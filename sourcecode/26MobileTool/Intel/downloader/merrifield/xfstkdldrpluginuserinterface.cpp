...
extern QString GlobalMedfieldMessageString;
extern QString GlobalMedfieldStatusValue;
extern int GlobalMedfieldProgressValue;
extern QStringList GlobalMedfieldStatusLog;
...
void attachFunction(void* clientData)
{
    ...
    int numtargets = plugin->socDevsCRC.number_devices + plugin->socDevsMerr.number_devices + plugin->socDevsMoor.number_devices;
    plugin->CurrentState.NumberOfAvailableTargets = numtargets;
    plugin->ui->DeviceStatus->setText(QString("MERRIFIELD, MOOREFIELD, & CARBONCANYON TARGETS DETECTED: %1").arg(numtargets));
    ...
    ui(new Ui::XfstkDldrPluginUserInterface)
{
    ...
    memset(&socDevsMerr,0,sizeof(socDevsMerr));
    memset(&socDevsMoor,0,sizeof(socDevsMoor));
    memset(&socDevsCRC,0,sizeof(socDevsCRC));
...
#ifdef XFSTK_OS_WIN
    this->devDetectWait = false;
    this->CallbackStructMerr.attach = attachFunction;
    this->CallbackStructMerr.detach = detachFunction;
    this->CallbackStructMerr.clientdata = this;
    this->CallbackStructMerr.device = &(this->socDevsMerr);
    this->CallbackStructMoor.attach = attachFunction;
    this->CallbackStructMoor.detach = detachFunction;
    this->CallbackStructMoor.clientdata = this;
    this->CallbackStructMoor.device = &(this->socDevsMoor);
    this->CallbackStructCRC.attach = attachFunction;
    this->CallbackStructCRC.detach = detachFunction;
    this->CallbackStructCRC.clientdata = this;
    this->CallbackStructCRC.device = &(this->socDevsCRC);
    this->dldrapi = new xfstkdldrapi();
    this->dldrapi->registerdevicecallback(&this->CallbackStructMerr,MERRIFIELD);
    this->dldrapi->registerdevicecallback(&this->CallbackStructMoor,MOOREFIELD);
    this->dldrapi->registerdevicecallback(&this->CallbackStructCRC,CARBONCANYON);
    this->CurrentState.USBTimeout = "10";
#else
    ...
    this->dldrapi = NULL;
#endif
    this->ui->DeviceStatus->setText(QString("MERRIFIELD, MOOREFIELD, & CARBONCANYON TARGETS DETECTED: 0"));
...
void XfstkDldrPluginUserInterface::OptionsInterfaceChanged(OptionsInterfaceState *State)
{
    if(!this->CurrentState.IsDownloadThreadDone) {
        ...
        if(State->ProvisionCount == QString("0")) {
            ...
            this->ui->ProvisionCount->setText(QString("Merrifield, Moorefield & Carbon Canyon Provisions Attempted: %1").arg(this->CurrentState.ProvisionCount));
        }
    ...
    this->SaveUserInterfaceStateToDisk(QString("Intel"),QString("Merrifield"));
...
void XfstkDldrPluginUserInterface::SaveUserInterfaceStateToDisk(QString Vendor, QString ProductPlugin)
{
    ...
    settings.beginGroup(ProductPlugin);
        ...
        settings.beginGroup("Download Options");
            ...
            settings.setValue("MiscDnxEnable", this->CurrentState.MiscDnxEnable);
            if(this->CurrentState.MiscDnxEnable) {
                settings.setValue("MiscDnXPath", this->CurrentState.MiscDnXPath);
            } else {
                settings.setValue("MiscDnXPath", QString("N/A"));
            }
        settings.endGroup();
    ...
    if(settings.status() == QSettings::NoError) {
        settings.beginGroup(ProductPlugin);
            ...
            settings.beginGroup("Download Options");
                ...
                this->CurrentState.MiscDnxEnable = settings.value("MiscDnxEnable", false).toBool();
                this->CurrentState.MiscDnXPath = settings.value("MiscDnXPath", "N/A").toString();
...
void XfstkDldrPluginUserInterface::ThreadComplete(bool result)
{
    ...
    this->CurrentState.IsDownloadThreadDone = true;
    if(this->CurrentState.StatusListPrevLength < GlobalMedfieldStatusLog.length()) {
        this->CurrentState.StatusListLocation = this->CurrentState.StatusListPrevLength;
        this->CurrentState.StatusListPrevLength = GlobalMedfieldStatusLog.length();
        if(this->CurrentState.ShowDownloadDetails) {
            int i = 0;
            for(int i = this->CurrentState.StatusListLocation; i < this->CurrentState.StatusListPrevLength;i++) {
                this->ui->StatusLog->addItem(QString("%1").arg(GlobalMedfieldStatusLog.at(i)));
            }
...
void XfstkDldrPluginUserInterface::UpdateUsbStatus()
{
    ...
    int numtargets = status.getavailabletargets(MERRIFIELD);
    numtargets += status.getavailabletargets(MOOREFIELD);
    numtargets += status.getavailabletargets(CARBONCANYON);
    this->ui->DeviceStatus->setText(QString("MERRIFIELD, MOOREFIELD, & CARBONCANYON TARGETS DETECTED: %1").arg(numtargets));
    if (numtargets != this->CurrentState.NumberOfAvailableTargets)
    {
        this->CurrentState.NumberOfAvailableTargets = numtargets;
        status.getavailabletargets(&targets);
        //socDevs = targets;
        if (targets.number_devices > 0)
        {
            QString logstr("Devices connected: ");
            char *p;
            int numbers = 0;
            int i;

            for (i = 0; i < targets.number_devices; i++)
            {
                p = targets.soc_device[i].usbsn;
                if (strlen(p) == 0) {
                    continue;
                }
                if (numbers > 0) {
            ...
            if (numbers > 0) {
                this->ui->StatusLog->addItem(logstr);
            }
        }
    }
}
...
void XfstkDldrPluginUserInterface::UpdateUiStatus()
{
    if(this->CurrentState.IsDownloadThreadDone) {
        ...
        this->ui->DownloadDetails->setText(QString("%1").arg(GlobalMedfieldStatusValue));
        this->ui->DownloadProgress->setValue(GlobalMedfieldProgressValue);
        if(this->CurrentState.StatusListPrevLength < GlobalMedfieldStatusLog.length()) {
            this->CurrentState.StatusListLocation = this->CurrentState.StatusListPrevLength;
            this->CurrentState.StatusListPrevLength = GlobalMedfieldStatusLog.length();
            if(this->CurrentState.ShowDownloadDetails) {
...
void XfstkDldrPluginUserInterface::BeginDownload()
{
    ...
    //this->MiscDnXPath = this->ui->MiscDnXPath->text();
    ...
    this->baMiscDnXPath = this->CurrentState.MiscDnXPath.toLatin1();
    ...
    this->c_str_MiscDnXPath = baMiscDnXPath.data();
    this->CurrentState.SuppressErrors = true;
    this->ui->StatusLog->clear();
    GlobalMedfieldStatusLog.clear();
    this->CurrentState.StatusListLocation = 0;
    ...
    if(FWDnX != "N/A" && FWImage != "N/A" && OSImage != "N/A") {
        ...
        //fw only
        //this->DownloaderInterface.downloadfw(c_str_FWDnX, c_str_FWImage,c_str_GPFlagsOverride);
        bFWOnly = true;
    ...
    this->CurrentState.SuppressErrors = false;
    GlobalMedfieldStatusValue = this->ui->DownloadDetails->text();
    ...
    if(this->CurrentState.IsDownloadThreadDone) {
        ...
        if(this->CurrentState.MiscDnxEnable == true) {
            xfstknewthreadobj.bMiscDnxEnable = true;
            xfstknewthreadobj.MiscDnXPath = this->CurrentState.MiscDnXPath;
        } else {
            xfstknewthreadobj.bMiscDnxEnable = false;
            xfstknewthreadobj.MiscDnXPath = "N/A";
        }
